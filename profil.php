...
$stmt_user = $pdo->prepare("SELECT * FROM users WHERE id_user = ?");
...
// Calculer le nombre total de likes de l'utilisateur
...
<div class="infoprofil">
    ...
    <div class="right">
        <div class="infos">
            ...
            <p>Followers : <!-- tu peux calculer les followers ici si tu veux --></p>
        ...
        <div class="btnprofil">
            ...
            <?php else: ?>
                <button class="btn subscribe-btn <?= $is_following ? 'following' : '' ?>" 
                        data-user="<?= $creator_id ?>">
                    <?= $is_following ? 'Abonné' : "S'abonner" ?>
                </button>
            <?php endif; ?>
        </div>
    ...
    <?php if (!empty($compositions)): ?>
        <div class="cards-container">
            <?php foreach ($compositions as $c): ?>
                ...
                ?>
                <div class="card">
                    <h3><?= htmlspecialchars($c['donut_name'], ENT_QUOTES) ?> <?= $c['type'] ? '('.htmlspecialchars($c['type'], ENT_QUOTES).')' : '' ?></h3>

                    <?php if (!empty($c['image']) || !empty($c['description'])): ?>
                        <div class="content">
                            <?php if (!empty($c['image'])): ?>
                                <img src="<?= htmlspecialchars($c['image'], ENT_QUOTES) ?>" alt="<?= htmlspecialchars($c['donut_name'], ENT_QUOTES) ?>">
                            <?php endif; ?>
